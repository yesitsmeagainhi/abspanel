...
    if (startAfterId) {
      const startDoc = await db.collection('lectures').doc(startAfterId).get();
      if (!startDoc.exists) {
...
    if (startAfterId) {
      const startDoc = await db.collection('announcements').doc(startAfterId).get();
      if (startDoc.exists) {
        q = q.startAfter(startDoc);
      }
    }
...
    let q = db.collection('banners').orderBy('order', 'asc');

    if (startAfterId) {
      const startDoc = await db.collection('banners').doc(startAfterId).get();
      if (startDoc.exists) {
        q = q.startAfter(startDoc);
      }
    }
